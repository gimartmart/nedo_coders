...
class X3m_ApplicationContext(discord.ApplicationContext):
    ...
    def get_comission_perc(
        self, member: discord.Member, return_comission_group=False
    ) -> Union[int, float]:
        ...
        zero_perc_role_id = walk_dict(self.server_settings, "roles.comission_zero_percent", 0)

        if zero_perc_role_id:
            zero_perc_role = self.guild.get_role(zero_perc_role_id)
            if zero_perc_role in member.roles:
                if return_comission_group:
                    return "zero"
                return 1
    ...
    async def _post_init(self) -> None:
        ...
        # -- \
        self.comission_group = self.get_comission_perc(self.author, return_comission_group=True)

        # comission_perc - is a comission, e.g. number * 0.95 means 5% comission.
        self.comission_perc = walk_dict(
            self.server_settings, "numbers.comission_" + self.comission_group, 1
        )
        # -- /
    ...
    def get_user_group(self, user_id: int):
        if str(user_id) in self.bot.global_settings.get("dev_ids", []):
            user_group = self.bot.permissions_name_number["dev"]
        else:
            user_group = self.server_settings.get("perm_groups", {}).get(
                str(user_id), None
    ...
    def default_embed(self, color: int = None) -> discord.Embed:
        embed = discord.Embed(color=color or Const.embed_color)
    ...
    async def user_has_enough_balance(
        ...
        res = (
            await self.fetch_user(
                fields=["balance" + balance_suffix],
                user_id=user_id,
                get_preferences=False,
            )
        ) or {}

        balance = res.get("balance" + balance_suffix, 0)
            ...
            if not x:
                await self.error(
                    self.locale(
                        ...
                        currency=self.get_emoji("currency" + balance_suffix),
                    ),
                    ephemeral=True,
                )
