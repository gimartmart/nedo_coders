...
class Const:
    ...
    embed_invis_color = 0x2B2D31
    embed_color_error = 0x2B2D31  # 0xFEB6B6
...
async def try_notify_user(bot, user, *args, **kwargs):
    if bot.store_exists(f"_internal.notify_fail.{user.id}"):
        return

    if not args and not kwargs:
        raise Exception("Empty args and kwargs.")

    try:
        await user.send(*args, **kwargs)
    except:
        bot.store_set(f"_internal.notify_fail.{user.id}", True, expire=7200)


async def try_notify_user_quest_achievement(
    bot,
    user: discord.User,
    guild_id: int,
    achievement: AwardType,
    award_balance: int = 500,
    award_cases: dict = {},
):
    """
    Увеличивает баланс и записывает транзакцию сама, выдает ачивку!
    :param award_cases: {"case": CasesEnum.timely_12h, "amount": 5}
    """
    settings = await bot.get_fetch_server_settings(guild_id)
    guild: discord.Guild = bot.get_guild(guild_id)
    loc = Locale(bot, settings.get("language", Config.get("default_language")))

    additional_text = ""

    if award_cases:
        additional_text = loc.locale(
            "quest_achievement_award_case",
            name=loc.locale("inventory_cases_name_" + award_cases["case"].name),
            amount=award_cases["amount"],
        )

    achievement_loc = loc.locale("quest_achievement_" + achievement.name)

    emoji_id = walk_dict(settings, "emojis.currency", 0)
    emoji = bot.get_emoji(emoji_id) or Const.default_emoji

    text = loc.locale(
        "quest_achievement_text",
        achievement=achievement_loc,
        award_balance=award_balance,
        additional_text=additional_text,
        emoji=emoji,
    )

    fields_guild = {"$inc": {"balance": award_balance}}

    if award_cases:
        fields_guild["$inc"][f"inventory.cases.{award_cases['case'].value}"] = (
            award_cases["amount"]
        )
    ...
    bot.save_transaction(
        ...
        user.id,
        award_balance,
        data={"achievement": achievement.value},
    )

    await bot.give_user_award(
        guild_id=guild_id,
        user_id=user.id,
        award=achievement,
    )
    ...
    embed = discord.Embed(color=Const.embed_color)
    embed.title = loc.locale("quest_achievement_title")
    embed.description = text

    icon_url = guild.icon.url if guild.icon else bot.user.display_avatar.url
    embed.set_thumbnail(url=icon_url)

    embed.set_footer(text=guild.name or "<guild>", icon_url=icon_url)
    await try_notify_user(bot, user, embed=embed)
